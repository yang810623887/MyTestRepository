...
namespace inviwo {
	...
	{
		for (auto observable : rhs.observables_)
		{
			addObservation(observable);
		}
	}
	...
	Observer& Observer::operator=(const Observer& that)
	{
		...
		{
			removeObservations();
			for (auto observable : that.observables_)
			{
				addObservation(observable);
			}
		}
	...
	void Observer::addObservation(ObservableInterface* observed)
	{
		...
		if (inserted.second)
		{
			observed->addObserverInternal(this);
		}
	}
