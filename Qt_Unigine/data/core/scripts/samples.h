...
class Interface
{
	...
	void init()
	{
		...
		forloop (int i = 0; engine.getNumDataPaths())
		{
			if (!directory.open(engine.getDataPath(i) + path))
				continue;

			forloop (int j = 0; directory.getNumFiles())
			{
				string name = basename(directory.getFileName(j));
		...
		{
			string files[0];
			engine.filesystem.getFileNames(files);
			string samples = relname(engine.getAppPath(), engine.getDataPath(0)) + path;
			foreach (string file; files)
			{
				if (strstr(file, samples) == 0 && extension(file) == "world")
					names.append(extension(basename(file), NULL));
			}
		}
	...
	Interface(string p, string default_w = "samples/samples")
	{
		path = p;
...
void createInterface(string p, string default_world = "samples/samples")
{
	interface = new Interface(p, default_world);
}
...
Body createBodyFracture(vec3 size, float density, float friction, float restitution, string material, string crack, Mat4 transform)
{
	...
	body.setSurfaceProperty("surface_base");
	mesh.setMaterial(material, "*");
	mesh.setSurfaceProperty("surface_base", "*");
	mesh.setTransform(transform);
	return body;
