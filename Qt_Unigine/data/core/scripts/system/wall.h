...
#ifdef HAS_APP_WALL || HAS_APP_PROJECTION || HAS_APP_SURROUND || HAS_APP_3D_SURROUND
...
void set_projection(int x, int y, mat4 projection, Mat4 modelview, Camera camera)
{
	...
	engine.wall.setReflectionMask(x, y, camera.getReflectionMask());
	...
	engine.projection.setReflectionMask(x, camera.getReflectionMask());
	...
	engine.surround.setReflectionMask(x, camera.getReflectionMask());
