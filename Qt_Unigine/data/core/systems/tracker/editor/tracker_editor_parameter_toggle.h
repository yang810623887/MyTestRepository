...
#ifndef __UNIGINE_TRACKER_EDITOR_PARAMETER_TOGGLE_H__
#define __UNIGINE_TRACKER_EDITOR_PARAMETER_TOGGLE_H__

namespace Unigine::Tracker
{
class TrackerEditorParameterToggle : TrackerEditorParameter
{
	using Unigine::Widgets;

	LineCurveToggle line_curve;

	TrackerParameterToggle::Track parameter_track;

	TrackerEditorParameterToggle(TrackerParameterToggle parameter, vec4 color)
		: TrackerEditorParameter(parameter)
	{
		parameter_track = new TrackerParameterToggle::Track();

		table_row = new TableRow(getParameterName(), EDITOR_TABLE_NUM_COLUMNS, 1, 1);
		table_row.setColor(color);
		table_row.setData(this);

		line_curve = new LineCurveToggle(color);
	...
	void updateTrack()
	{
		...
		foreach (LineKeyToggle key; line_curve.keys)
		{
	...
	void updateCurve()
	{
		...
		{
			LineKeyToggle line_key = new LineKeyToggle(key.time, key.value);
...
#endif /* __UNIGINE_TRACKER_EDITOR_PARAMETER_TOGGLE_H__ */
